...
export const placeBid = async (_: any, { bidAmount, artId }: any, context: any) => {
    try {
...
export const updateBidAmount = async (_: any, { bidId, bidAmount }: any, context: any) => {
    try {
        const user: any = Authenticate(context);
        ...
        const bid = await BidModel.findByIdAndUpdate(bidId, { bidAmount }, { new: true });
...
export const updateStartPrice = async (_: any, { artId }: any, context: any) => {
    try {
        ...
        const art = await ArtModel.findByIdAndUpdate(artId, { auctionStartPrice: highestBid[0].bidAmount }, { new: true });
